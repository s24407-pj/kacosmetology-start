...
has no general API/data-service layer: authored business data is compiled from
TypeScript in `src/data/`. Evidence: `src/routes/index.tsx`, F-008.
...
- `promotion.ts` owns campaigns, applicability, active selection, and
  ...
  production configuration gate. Evidence: F-001, F-002, F-004, F-005, F-007.
...
Evidence: F-010 and its E2E tests.
...
fire-and-forget. Evidence: F-012.
...
- Looks wrong at first glance; intentional because historical price rows repeat
  ...
  today's catalog or campaign configuration. Evidence: F-002 adversarial review.
...
- Looks wrong at first glance; intentional because Mobile Safari skips only the
  ...
  recovery. Evidence: `audit/deviations/F-003-minor-webkit-recovery-harness.json`.
- The E2E client-ready document attribute is a test synchronization contract,
  ...
  Evidence: F-010 readiness validation.
...
No out-of-repository consumers were reported during this audit; that means none
are known, not that none exist. Re-check before changing exported, DOM,
analytics, generated-file, or deployment contracts. Evidence: approved H-05.
