...
describe('price history helpers', () => {
  ...
  it('records price changes in chronological order', () => {
    recordPriceChange(SERVICE_NAME, 100, new Date('2024-01-10T10:00:00Z'))
    recordPriceChange(SERVICE_NAME, 90, new Date('2024-01-02T10:00:00Z'))

    expect(getPriceHistory(SERVICE_NAME)).toEqual([
  ...
  it('normalises string date values into ISO strings', () => {
    recordPriceChange(SERVICE_NAME, 150, '2024-03-01T08:30:00+01:00')
  ...
  it('returns a defensive copy of the history list', () => {
    recordPriceChange(SERVICE_NAME, 95, new Date('2024-04-01T09:00:00Z'))

    const history = getPriceHistory(SERVICE_NAME)
  ...
  it('returns the lowest price within the provided window', () => {
    recordPriceChange(SERVICE_NAME, 120, new Date('2023-12-10T10:00:00Z'))
    recordPriceChange(SERVICE_NAME, 110, new Date('2024-01-05T10:00:00Z'))
    recordPriceChange(SERVICE_NAME, 90, new Date('2024-01-20T10:00:00Z'))
  ...
  it('treats entries on the window boundary as part of the range', () => {
    ...
    recordPriceChange(SERVICE_NAME, 150, threshold)
    recordPriceChange(SERVICE_NAME, 200, new Date('2024-01-15T00:00:00Z'))

    expect(getLowestPriceInLastDays(SERVICE_NAME, 30, now)).toBe(150)
  })
  ...
  it('considers the price active at the beginning of the window', () => {
    recordPriceChange(SERVICE_NAME, 100, new Date('2024-01-01T10:00:00Z')) // Price before window
    recordPriceChange(SERVICE_NAME, 120, new Date('2024-01-15T10:00:00Z')) // Price inside window
  ...
  it('selects the lowest price from within the window if it is lower', () => {
    recordPriceChange(SERVICE_NAME, 100, new Date('2024-01-01T10:00:00Z')) // Price before window
    recordPriceChange(SERVICE_NAME, 80, new Date('2024-01-15T10:00:00Z')) // Price inside window
    recordPriceChange(SERVICE_NAME, 120, new Date('2024-01-20T10:00:00Z')) // Price inside window
  ...
  it('returns undefined for services without history', () => {
    expect(getLowestPriceInLastDays('Unknown Service')).toBeUndefined()
    expect(getPriceHistory('Unknown Service')).toEqual([])
  })
