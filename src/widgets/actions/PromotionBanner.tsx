...
export default function PromotionBanner() {
  ...
  useEffect(() => {
    if (!promotion || isDismissed) return
  ...
  const bannerMessage = useMemo(() => {
    ...
    return `Promocja! - ${getPromotionScopeDescription(promotion)} ${formatPromotionDeadline(promotion)}.`
  }, [promotion])
  ...
  return (
    ...
    >
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-2 px-4 py-2 pr-11 text-sm sm:flex-row sm:items-center sm:justify-center sm:gap-4 sm:px-6 sm:pr-16 sm:text-base lg:px-8">
        <div className="min-w-0 flex items-start gap-2 text-left sm:items-center sm:text-center">
          <span className="mt-0.5 inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-md bg-white/15 sm:mt-0 sm:h-6 sm:w-6">
            <Percent className="h-3.5 w-3.5 sm:h-4 sm:w-4" aria-hidden="true" />
          </span>
